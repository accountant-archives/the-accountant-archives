...
function FactsPage({ member }: { member: Member | null }) {
  ...
  const children = new Map<string, FactNode[]>(); for (const fact of facts) if (fact.parent_id) children.set(fact.parent_id, [...(children.get(fact.parent_id) ?? []), fact]); const roots = facts.filter((fact) => !fact.parent_id || !facts.some((candidate) => candidate.id === fact.parent_id))
  function FactCard({ fact, depth = 0 }: { fact: FactNode; depth?: number }) {
    ...
    return <li className="fact-tree-item" style={{ '--fact-depth': depth } as React.CSSProperties}><article className="fact-card sourced"><header><span className="fact-status confirmed"><Check size={14} /> Sourced</span><small>{author ? `${author.display_name} · @${author.handle}` : 'Archive contributor'} · {relativeDate(fact.created_at)}</small></header><h2>{fact.title}</h2><p>{fact.body}</p>{evidence.map((source) => { const story = stories.find((entry) => entry.id === source.story_id); return <blockquote key={source.id}><span>Movie #{story ? String(story.film_number).padStart(3, '0') : '—'} · {story?.title ?? 'Published source'}</span><p>“{source.quoted_text}”</p>{source.note && <small>{source.note}</small>}</blockquote> })}<footer><div className="fact-reactions"><button className={mine.get(fact.id) === 1 ? 'selected' : ''} onClick={() => void react(fact, 1)} title="Support this sourced fact"><ArrowUp size={15} /> {metric.supports}</button><button className={mine.get(fact.id) === -1 ? 'selected' : ''} onClick={() => void react(fact, -1)} title="Dispute this fact"><ArrowDown size={15} /> {metric.disputes}</button><span>{metric.sources} source{metric.sources === 1 ? '' : 's'}</span></div>{member && <button className="button ghost slim" onClick={() => setEvidenceFor(evidenceFor === fact.id ? null : fact.id)}>Add evidence</button>}</footer>{evidenceFor === fact.id && <div className="evidence-drawer"><PassagePicker stories={stories} storyId={evidenceStoryId} setStoryId={setEvidenceStoryId} quote={evidenceQuote} setQuote={setEvidenceQuote} /><input value={evidenceNote} onChange={(event) => setEvidenceNote(event.target.value)} placeholder="Why this passage matters (optional)" maxLength={400} /><button className="button primary slim" onClick={() => void addEvidence()}>Add this source</button></div>}</article>{childFacts.length > 0 && <ol className="fact-tree">{childFacts.map((child) => <FactCard key={child.id} fact={child} depth={depth + 1} />)}</ol>}</li>
  }
  return <section className="facts-page section-wrap"><div className="facts-heading"><div><p className="eyebrow"><span /> Shared evidence</p><h1>The <i>Fact Sheet.</i></h1><p>A living collection of sourced details. Anyone signed in can publish a fact, add another passage, or challenge it with the group.</p></div><div className="facts-actions">{member ? <button className="button primary" onClick={() => setComposerOpen((open) => !open)}><PenLine size={16} /> Add a sourced fact</button> : <span>Sign in to contribute</span>}</div></div>{composerOpen && <section className="fact-composer fact-composer-premium"><div><p className="eyebrow"><span /> Source-first entry</p><h2>Start with the evidence.</h2><p>Choose a published story, then highlight the exact passage which supports the fact. That keeps the record checkable.</p></div><div className="fact-form"><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="What is this fact called?" maxLength={120} /><textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder="Explain the fact in plain, specific language…" maxLength={6000} /><label>Filed under<select value={parentId} onChange={(event) => setParentId(event.target.value)}><option value="">Top-level entry</option>{facts.map((fact) => <option key={fact.id} value={fact.id}>{fact.title}</option>)}</select></label><PassagePicker stories={stories} storyId={storyId} setStoryId={setStoryId} quote={quote} setQuote={setQuote} /><input value={note} onChange={(event) => setNote(event.target.value)} placeholder="Why this source supports the fact (optional)" maxLength={400} /><div><button className="button primary slim" disabled={submitting} onClick={() => void publishFact()}>{submitting ? 'Publishing…' : 'Publish sourced fact'} <Send size={14} /></button><button className="button ghost slim" onClick={() => setComposerOpen(false)}>Cancel</button></div></div></section>}<div className="facts-key"><span className="confirmed">Sourced entry</span><span>Support or dispute an entry with the group.</span></div>{loading ? <div className="loading-card"><LoaderCircle className="spin" /> Loading the record…</div> : roots.length ? <ol className="fact-tree fact-tree-root">{roots.map((fact) => <FactCard key={fact.id} fact={fact} />)}</ol> : <div className="empty-canon fact-empty"><p className="eyebrow"><span /> Blank reference</p><h2>Be the first to add a <i>sourced detail.</i></h2><p>Pick any published story and select the passage that should enter the record.</p></div>}{message && <p className="fact-message">{message}</p>}</section>
}
...
function StoryDiscussion({ storyId, onOpenAuthor }: { storyId: string; onOpenAuthor?: (id: string) => void }) {
  const [open, setOpen] = useState(true); const [body, setBody] = useState(''); const [comments, setComments] = useState<Array<{ id: string; body: string; created_at: string; author?: { id: string; handle: string; display_name: string; ledger_balance: number } }>>([]); const [message, setMessage] = useState(''); const [battle, setBattle] = useState<{ id: string; canon_story_id: string; challenger_story_id: string; closes_at: string; canon_votes: number; challenger_votes: number; mine?: string } | null>(null)
...
function ProfileSheet({ member, close, logout, onSaved }: { member: Member; close: () => void; logout: () => void; onSaved: (displayName: string) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={close}><section className="profile-sheet" role="dialog" aria-modal="true" aria-label="Your profile" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={close}><X size={18} /></button><div className="profile-heading"><span className="profile-avatar">{displayName.slice(0, 1)}</span><div><p className="eyebrow"><span /> Your account</p><h2>{displayName}</h2><p>@{member.handle}</p></div></div><div className="profile-stats"><div><small>Sparkle points</small><b><Sparkles size={15} /> {formatNumber(member.ledger)}</b></div><div><small>Archive role</small><b>{member.role === 'admin' ? 'Administrator' : member.role === 'moderator' ? 'Moderator' : 'Writer'}</b></div></div>{editing ? <div className="profile-editor"><label>Display name<input value={displayName} maxLength={80} onChange={(event) => setDisplayName(event.target.value)} /></label><label>About you<textarea value={bio} maxLength={500} onChange={(event) => setBio(event.target.value)} placeholder="A sentence or two about the kind of record you keep…" /></label><div><button className="button primary slim" disabled={saving} onClick={() => void save()}>{saving ? 'Saving…' : 'Save profile'}</button><button className="button ghost slim" onClick={() => setEditing(false)}>Cancel</button></div></div> : <><p className="profile-bio">{bio || 'No note in the margin yet.'}</p><button className="text-button profile-edit" onClick={() => setEditing(true)}><FilePenLine size={15} /> Edit profile</button></>}<p className="profile-note">Your account keeps your drafts, votes, and stories tied to the archive. Signing out only affects this device.</p>{message && <small className="profile-message">{message}</small>}<button className="profile-logout" onClick={() => { close(); void logout() }}>Sign out <ArrowRight size={16} /></button></section></div>
}
