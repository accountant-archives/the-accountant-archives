...
export function App() {
  ...
  async function saveDraft(silent = false): Promise<string | null> {
    ...
    setDraftSaving(true)
    const payload = { title: draft.title.trim() || 'Untitled ledger entry', body_markdown: draft.body, film_number: focusedFilm, author_id: member.id }
    ...
    setDraftId(id)
    if (!silent) notify('Draft saved to your ledger.')
    return id
  }
  ...
  async function submitDraft() {
    ...
    notify('Submitted. The ledger has recorded your entry.')
  ...
  async function reactToStory(story: Story, value: 1 | -1) {
    ...
    else { notify(value === 1 ? 'Credited to the story ledger.' : 'Recorded as a concern.'); void loadFilm(focusedFilm) }
  }

  async function bookmarkStory(story: Story) {
    if (!member || !supabase) { setSignInOpen(true); return }
    const { error } = await supabase.from('bookmarks').upsert({ story_id: story.id, user_id: member.id }, { onConflict: 'story_id,user_id' })
    if (error) notify(error.message); else notify('Saved to your case file.')
  }
  ...
  return (
    <div className="app-shell">
      ...
      <footer className="footer"><span>800 films. One debt. An infinite paper trail.</span><span>Built for the record keepers.</span></footer>
  ...
  return <>
    <section className="hero">
      <div className="hero-copy">
        <p className="eyebrow"><span /> Community canon project</p>
        <h1>Every debt<br /><i>has a sequel.</i></h1>
        <p className="hero-lede">The open archive for an 800-film saga about an accountant, an unpaid balance, and the increasingly unreasonable ways to collect it.</p>
        <div className="hero-actions"><button className="button primary" onClick={onBrowse}>Enter the archive <MoveRight size={17} /></button><button className="button ghost" onClick={onWrite}><FilePenLine size={17} /> Write movie #1</button></div>
        <div className="hero-stats"><div><b>800</b><span>films on file</span></div><div><b>∞</b><span>possible canon</span></div><div><b>7 days</b><span>to settle a challenge</span></div></div>
      </div>
    ...
    <section className="home-grid section-wrap">
      <div className="section-title"><p className="eyebrow"><span /> Start anywhere</p><h2>The ledger is <i>alive.</i></h2><p>Read canon, argue for a better version, or leave the next note on the debtor’s desk.</p></div>
      <div className="feature-cards">
        <button className="feature-card red" onClick={() => onOpenFilm(1)}><span className="card-no">01</span><BookOpen /><h3>Read the record</h3><p>Every film begins with an official prompt and can grow a messy, magnificent canon.</p><MoveRight /></button>
        <button className="feature-card gold" onClick={onBrowse}><span className="card-no">02</span><Gavel /><h3>Challenge canon</h3><p>When an entry no longer balances, put your version before the archive.</p><MoveRight /></button>
        <button className="feature-card blue" onClick={onWrite}><span className="card-no">03</span><PenLine /><h3>Leave an entry</h3><p>The writing desk knows the film before and after. Continuity has never felt so good.</p><MoveRight /></button>
      </div>
    </section>
    <section className="era-strip section-wrap"><div><p className="eyebrow"><span /> The chronology</p><h2>Six eras. No clean exits.</h2></div><div className="mini-timeline">{ERAS.map((era) => <button key={era.slug} style={{ '--era-colour': era.accent, flex: era.end_movie - era.start_movie + 1 } as React.CSSProperties} onClick={() => onOpenFilm(era.start_movie)}><b>{era.name}</b><small>#{era.start_movie}–{era.end_movie}</small></button>)}</div></section>
  </>
}
...
function Archive({ films, search, setSearch, eraFilter, setEraFilter, onOpenFilm }: { films: Film[]; search: string; setSearch: (value: string) => void; eraFilter: string; setEraFilter: (value: string) => void; onOpenFilm: (number: number) => void }) {
  ...
  return <section className="archive section-wrap">
    <div className="archive-heading"><div><p className="eyebrow"><span /> The full paper trail</p><h1>Browse the <i>800.</i></h1><p>Titles get smaller. Consequences do not.</p></div><div className="archive-amount">{formatNumber(visible.length)} <span>entries found</span></div></div>
    <div className="archive-tools"><label className="search"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find a title, film number, or bad decision" /><kbd>⌘ K</kbd></label><div className="era-filters"><button className={eraFilter === 'all' ? 'selected' : ''} onClick={() => setEraFilter('all')}>All</button>{ERAS.map((era) => <button key={era.slug} className={eraFilter === era.slug ? 'selected' : ''} onClick={() => setEraFilter(era.slug)}>{era.name}</button>)}</div></div>
...
function Timeline({ eras, focus, setFocus, onOpenFilm }: { eras: Era[]; focus: string | null; setFocus: (value: string | null) => void; onOpenFilm: (number: number) => void }) {
  ...
  return <section className="timeline-page section-wrap">
    <div className="timeline-heading"><div><p className="eyebrow"><span /> Continuity map</p><h1>Follow the <i>money.</i></h1><p>The eras are rules of engagement, not a cage. They can be reshaped by the archive’s administrator as the canon evolves.</p></div><div className="timeline-key"><span><i className="dot canon" />Canon</span><span><i className="dot challenger" />Open challenge</span><span><i className="dot unclaimed" />Awaiting story</span></div></div>
    ...
    <div className="timeline-detail">{active ? <><span className="era-swatch" style={{ background: active.accent }} /><div><p className="eyebrow">#{active.start_movie}–#{active.end_movie}</p><h2>{active.name}</h2><p>{active.description}</p></div><aside><b>Writer’s brief</b><p>{active.writing_guidelines}</p><button className="text-button" onClick={() => onOpenFilm(active.start_movie)}>Open first film <ArrowRight size={15} /></button></aside></> : <><Grid3X3 size={28} /><div><h2>Click an era to inspect its writing brief.</h2><p>The timeline stays deliberately editable: the debt may need a new chapter.</p></div></>}</div>
    <div className="timeline-callout"><div><Sparkles /><h2>Grothkin begins at <i>movie #220.</i></h2><p>From there, fractions are no longer just math. They are artefacts, oaths, and a bad idea with an accounts department.</p></div><button className="button ghost" onClick={() => onOpenFilm(220)}>Open Grothkin Lore <ArrowRight size={16} /></button></div>
  </section>
}
...
function FilmPage({ film, stories, loading, onBack, onWrite, onReact, onBookmark }: { film: Film; stories: Story[]; loading: boolean; onBack: () => void; onWrite: () => void; onReact: (story: Story, value: 1 | -1) => void; onBookmark: (story: Story) => void }) {
  ...
  return <section className="film-page section-wrap">
    ...
    <div className="film-layout"><aside className="continuity-card"><p className="eyebrow">Continuity</p><div><small>Before</small><b>#{film.number === 1 ? '—' : film.number - 1}</b><span>{film.number === 1 ? 'No earlier entry.' : titleForFilm(film.number - 1)}</span></div><div className="current"><small>Now</small><b>#{film.number}</b><span>{film.title}</span></div><div><small>After</small><b>#{film.number === 800 ? '—' : film.number + 1}</b><span>{film.number === 800 ? 'The ledger is closed.' : titleForFilm(film.number + 1)}</span></div><hr /><p>{film.era.writing_guidelines}</p></aside>
      <div className="story-column">{loading ? <div className="loading-card"><LoaderCircle className="spin" />Opening the case file…</div> : canon ? <StoryCard story={canon} featured onReact={onReact} onBookmark={onBookmark} /> : <EmptyCanon film={film} onWrite={onWrite} />}{challengers.length > 0 && <section className="challenge-stack"><div className="stack-heading"><div><p className="eyebrow"><span /> Live decision</p><h2>Canon is under review.</h2></div><span className="vote-window">6d 18h remaining</span></div>{challengers.map((story) => <StoryCard key={story.id} story={story} challenge onReact={onReact} onBookmark={onBookmark} />)}</section>}<section className="archive-note"><LockKeyhole size={17} /><div><b>Nothing disappears.</b><p>Stories that do not become canon remain in the archive, with their history intact.</p></div></section></div>
    </div>
  </section>
}

function EmptyCanon({ film, onWrite }: { film: Film; onWrite: () => void }) { return <div className="empty-canon"><div className="empty-stamp">OPEN FILE</div><p className="eyebrow"><span /> No canon established</p><h2>This movie is waiting for its <i>first account.</i></h2><p>Read the brief, check the films either side, and make the first entry on the record. The archive will remember who opened the file.</p><button className="button primary" onClick={onWrite}>Write movie #{film.number} <ArrowRight size={16} /></button></div> }
...
function WritingStudio({ film, draft, setDraft, previewing, setPreviewing, saving, onSave, onSubmit, onBack, onFormat }: { film: Film; draft: { title: string; body: string }; setDraft: React.Dispatch<React.SetStateAction<{ title: string; body: string }>>; previewing: boolean; setPreviewing: (value: boolean) => void; saving: boolean; onSave: () => void; onSubmit: () => void; onBack: () => void; onFormat: (before: string, after?: string) => void }) {
  ...
  return <section className="studio"><header className="studio-bar"><button className="back-button" onClick={onBack}><ArrowLeft size={16} /> Movie #{film.number}</button><div className="studio-state">{saving ? <><LoaderCircle className="spin" size={15} /> Saving</> : <><span /> Autosave on</>}</div><div><button className="button slim ghost" onClick={onSave}>Save draft</button><button className="button slim primary" onClick={onSubmit} disabled={wordCount < 300}>Submit to archive <Send size={15} /></button></div></header><div className="studio-layout"><aside className="studio-context"><p className="eyebrow">Your assignment</p><div className="current-film"><span style={{ background: film.era.accent }} /> <small>Movie #{String(film.number).padStart(3, '0')}</small><h2>{film.title}</h2><p>{film.official_description}</p></div><div className="context-neighbours"><button><small>PREVIOUS</small><b>#{film.number === 1 ? '—' : film.number - 1}</b><span>{film.number === 1 ? 'No earlier record' : titleForFilm(film.number - 1)}</span></button><button><small>NEXT</small><b>#{film.number === 800 ? '—' : film.number + 1}</b><span>{film.number === 800 ? 'End of ledger' : titleForFilm(film.number + 1)}</span></button></div><div className="brief-box"><p className="eyebrow">{film.era.name} brief</p><p>{film.era.writing_guidelines}</p></div></aside><div className="editor-area"><div className="editor-top"><div><p className="eyebrow">Drafting new story</p><input value={draft.title} onChange={(event) => setDraft((old) => ({ ...old, title: event.target.value }))} placeholder="Give this entry a title" aria-label="Story title" /></div><div className="editor-mode"><button className={!previewing ? 'selected' : ''} onClick={() => setPreviewing(false)}>Write</button><button className={previewing ? 'selected' : ''} onClick={() => setPreviewing(true)}>Preview</button></div></div>{!previewing && <div className="format-bar"><button onClick={() => onFormat('## ')}>H2</button><button onClick={() => onFormat('**', '**')}><b>B</b></button><button onClick={() => onFormat('*', '*')}><i>I</i></button><button onClick={() => onFormat('> ')}>Quote</button><button onClick={() => onFormat('- ')}>List</button><span>Markdown supported</span></div>}{previewing ? <article className="preview-prose"><h1>{draft.title || 'Untitled ledger entry'}</h1><p>{draft.body || 'Your story will appear here exactly as the archive will read it.'}</p></article> : <textarea id="story-body" value={draft.body} onChange={(event) => setDraft((old) => ({ ...old, body: event.target.value }))} placeholder="The desk lamp is on. The debt is not going anywhere." spellCheck /> }<footer className="editor-footer"><div><b>{formatNumber(wordCount)}</b> words · {readingTime(wordCount)} min read<div className="word-meter"><i style={{ width: `${complete}%` }} /></div><small>{wordCount < 300 ? `${300 - wordCount} words until submission` : 'Submission threshold met'}</small></div><p>Every save creates a revision. <span>Nothing gets lost.</span></p></footer></div></div></section>
}
...
function AdminRoom({ member, eras, setEras, queue, onModerate, notify }: { member: Member | null; eras: Era[]; setEras: React.Dispatch<React.SetStateAction<Era[]>>; queue: Story[]; onModerate: (story: Story, action: 'approve_canon' | 'archive' | 'reject') => void; notify: (message: string) => void }) {
  ...
  if (!member || member.role !== 'admin') return <section className="restricted section-wrap"><ShieldCheck size={34} /><h1>Control room sealed.</h1><p>Only an administrator can adjust canon infrastructure.</p></section>
  async function saveEra() {
    ...
    if (error) notify(error.message); else { setEras((all) => all.map((era) => era.id === editing.id ? editing : era)); notify('Era saved and film assignments synchronised.') }
  }
