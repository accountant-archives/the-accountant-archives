...
type FlameTier = 'ember' | 'goldflame' | 'arcane'

function flameTierFor(points: number): FlameTier | null {
  if (points >= 1000) return 'arcane'
  if (points >= 500) return 'goldflame'
  if (points >= 100) return 'ember'
  return null
}

function rankFor(points: number) {
  const flameTier = flameTierFor(points)
  if (flameTier) return flameTier
...
function FlameFrame({ tier, surface, className, children }: { tier: FlameTier; surface: 'pill' | 'leaderboard' | 'bio'; className: string; children: ReactNode }) {
  ...
  const isBio = surface === 'bio'
  const height = isPill ? 30 : isBio ? 30 : tier === 'ember' ? 40 : tier === 'goldflame' ? 49 : 58
  return <FlameWrap className={classNames(className, `flame-${tier}`)} color={colour} intensity={tier === 'ember' ? 0.82 : tier === 'goldflame' ? 1.35 : 1.65} height={height} spread={isPill ? 5 : isBio ? 5 : 7} radius={isPill ? 999 : 0} speed={tier === 'ember' ? 0.2 : tier === 'goldflame' ? 0.32 : 0.42} scale={tier === 'arcane' ? 0.7 : 0.58} turbulence={tier === 'ember' ? 0.28 : 0.42} turbulenceScale={0.55} turbulenceReach={isPill ? 8 : isBio ? 9 : 13} sparks={tier === 'ember' ? 0.35 : tier === 'goldflame' ? 0.8 : 1.15} sparkSize={0.28} sparkDensity={tier === 'ember' ? 0.45 : tier === 'goldflame' ? 0.65 : 0.85} sparkSpeed={0.7} rim={tier === 'ember' ? 0.9 : tier === 'goldflame' ? 1.35 : 1.65} melt={isBio ? 0 : 1.1} distortion={isBio ? 0 : 2} smoke={tier === 'arcane' ? 0.38 : 0.16} ember={tier === 'ember' ? 0.65 : tier === 'goldflame' ? 1 : 1.25} scorch={0}>{children}</FlameWrap>
}

function ProfileBio({ points, children }: { points: number; children: ReactNode }) {
  const tier = flameTierFor(points)
  const bio = <p className={classNames('profile-bio', tier && `profile-bio-${tier}`)}>{children}</p>
  return tier ? <FlameFrame tier={tier} surface="bio" className="profile-bio-wrap">{bio}</FlameFrame> : bio
}
...
function Leaderboard({ onOpenAuthor }: { onOpenAuthor: (id: string) => void }) {
  ...
  const flameTiers: Array<{ tier: FlameTier; members: typeof members }> = ['arcane', 'goldflame', 'ember'].map((tier) => ({ tier: tier as FlameTier, members: members.filter((member) => flameTierFor(member.ledger_balance) === tier) })).filter((entry) => entry.members.length)
  const standardMembers = members.filter((member) => !flameTierFor(member.ledger_balance))
...
function RankedMemberPill({ member, onOpen }: { member: Member; onOpen: () => void }) {
  ...
  const tier = flameTierFor(member.ledger)
  return tier ? <FlameFrame tier={tier} surface="pill" className="member-flame-wrap">{pill}</FlameFrame> : pill
}
