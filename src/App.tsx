...
function ProfileSheet({ member, close, logout, onSaved }: { member: Member; close: () => void; logout: () => void; onSaved: (displayName: string) => void }) {
  ...
  const sheet = <section className={classNames('profile-sheet', `rank-${rankFor(member.ledger)}`)} role="dialog" aria-modal="true" aria-label="Your profile" onMouseDown={(event) => event.stopPropagation()}>
    <button className="modal-close" onClick={close}><X size={18} /></button>
    <div className="profile-heading"><span className={classNames('profile-avatar', `rank-${rankFor(member.ledger)}`)}>{displayName.slice(0, 1)}</span><div><p className="eyebrow"><span /> Your account</p><h2>{displayName}</h2><p>@{member.handle}</p></div></div>
    <div className="profile-stats"><div><small>Sparkle points</small><b><Sparkles size={15} /> {formatNumber(member.ledger)}</b></div><div><small>Archive role</small><b>{member.role === 'admin' ? 'Administrator' : member.role === 'moderator' ? 'Moderator' : 'Writer'}</b></div></div>
...
function ContributorProfileSheet({ profileId, close, onOpenFilm }: { profileId: string; close: () => void; onOpenFilm: (film: number) => void }) {
  ...
  const sheet = <section className={classNames('profile-sheet', 'contributor-sheet', profile && `rank-${rankFor(profile.ledger)}`)} role="dialog" aria-modal="true" aria-label="Contributor profile" onMouseDown={(event) => event.stopPropagation()}>
    <button className="modal-close" onClick={close}><X size={18} /></button>
